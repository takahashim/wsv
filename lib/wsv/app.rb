  ...
  class App
    ...
    def call(request)
      unless ALLOWED_METHODS.include?(request.method)
        return Response.text(405, headers: {"Allow" => "GET, HEAD"})
      end

      raw_path, query = request.target.split("?", 2)
      result = @resolver.resolve(raw_path)

      return Response.text(result.status, head: request.head?) if result.error?
      return Response.redirect(redirect_location(raw_path, query), head: request.head?) if result.redirect?

      Response.file(result.file, head: request.head?)
    end
